...
**미디어 업로드 → 플레이어 전환 흐름:**
1. 초기 상태: 업로드 드롭존 표시 (드래그 앤 드롭 + 미디어 라이브러리 선택 버튼)
2. 영상 업로드 완료 → 자동으로 VideoPlayer + WaveformViewer로 전환
3. 이미지 업로드 완료 → 자동으로 이미지 갤러리 뷰로 전환
4. 미디어 교체/추가: 우하단 "파일 추가/교체" 버튼으로 언제든 변경 가능
5. 편집 모드 진입 시: 기존 미디어가 있으면 바로 플레이어/갤러리 표시

### 2-3. 콘텐츠 정보 (모달)

**도구 바가 아닌 헤더 버튼으로 진입하는 모달.** 제목/본문/해시태그는 한 번 작성하면 자주 수정하지 않으므로 모달로 분리.

```
┌─── 콘텐츠 정보 ─────────────────────────────── [×] ──┐
│                                                       │
│ 제목 *                                                │
│ [콘텐츠 제목 입력________________________] [AI 제안 ▾] │
│                                                       │
│ 본문/설명문                                            │
│ [콘텐츠 본문을 작성하세요                          ]   │
│ [                                                  ]   │
...
- 헤더 영역에 [📝 콘텐츠 정보] 버튼 → 클릭 시 모달 오픈
- 헤더에 현재 제목 요약 표시 (입력된 경우)
...
**도구 바가 아닌 헤더 버튼으로 진입하는 모달.** 플랫폼 선택과 변형본 설정은 초기 1회 설정 후 자주 바꾸지 않으므로 모달로 분리.
...
- 헤더 영역에 [📱 플랫폼 설정] 버튼 → 클릭 시 모달 오픈
- 헤더에 선택된 플랫폼 태그 요약 표시 (예: `YouTube · Instagram`)
- width="80vw"로 충분한 공간 확보
...
| 파일 | 유형 | 설명 |
|---|---|---|
| `features/contents/components/ContentInfoModal.tsx` | **모달** | 콘텐츠 정보 (제목/본문/해시태그/예약) — 헤더 버튼으로 진입 |
| `features/contents/components/PlatformSettingsModal.tsx` | **모달** | 플랫폼 선택/채널/변형본 — 헤더 버튼으로 진입 |
| `features/contents/components/MediaMainArea.tsx` | 영역 | 상단 미디어 영역 (업로드→플레이어 자동전환) |
| `features/contents/components/SubtitlePanel.tsx` | 패널 | 자막 편집 인라인 패널 (SubtitleEditorPage 리팩터링) |
| `features/contents/components/ShortformPanel.tsx` | 패널 | 숏폼 추출 인라인 패널 (ShortformEditorPage 리팩터링) |
| `features/contents/components/EffectsPanel.tsx` | 패널 | 효과음 패널 (신규) |
| `features/contents/components/EmojiPanel.tsx` | 패널 | 이모지/스티커 패널 (신규) |
| `features/contents/components/AiAssistantPanel.tsx` | 패널 | AI 어시스턴트 통합 패널 (톤변환/검수/번역/썸네일) |
...
- ContentEditorPage 내 AiSuggestionPanel 직접 사용 (ContentInfoModal로 이동)
- ContentEditorPage 내 기존 인라인 폼 필드 (모달로 이동했으므로)
...
1. `ContentInfoModal.tsx` 생성 — 기존 제목/본문/해시태그/예약 폼을 모달로 추출
2. `AiAssistantPanel.tsx` 생성 — 기존 AI 어시스턴트 카드 + 모달들 추출
3. `PlatformSettingsModal.tsx` 생성 — 플랫폼 선택/커스터마이즈/VariantEditor를 모달로 추출
4. AI 버그 수정: `getContentText` 개선, 중복 버튼 제거, 해시태그 결과 표시

### Sprint 2: 영상 중심 레이아웃
5. `MediaMainArea.tsx` 생성 — 업로드 드롭존 → VideoPlayer/이미지 갤러리 자동 전환
6. ContentEditorPage 재작성 — 헤더(모달 버튼) + 미디어 메인 + Segmented 도구 바 + 패널 시스템
7. 미디어 타입 감지 + 업로드 완료 시 자동 전환 로직
...
효과음과 이모지 데이터는 콘텐츠의 `metadata` JSONB 필드에 저장합니다.
별도 테이블 추가 없이 기존 구조를 활용합니다.

### 8-1. 저장 구조

```typescript
// ContentRecord.metadata에 저장
interface ContentMetadata {
  // 기존 필드들...

  // 효과음 타임라인
  sound_effects?: SoundEffectEntry[];

  // 이모지 타임라인
  emoji_overlays?: EmojiOverlayEntry[];
}

interface SoundEffectEntry {
  id: string;           // uuid
  timestamp: number;    // 초 단위 (0.0 ~)
  type: string;         // 'notification' | 'applause' | 'transition' | 'whoosh' | ...
  label: string;        // 사용자 표시명
  duration?: number;    // 효과음 길이 (초)
  ai_suggested: boolean; // AI 추천 여부
}

interface EmojiOverlayEntry {
  id: string;           // uuid
  timestamp: number;    // 초 단위
  emoji: string;        // '😊', '🎉' 등
  matched_subtitle?: string; // 매칭된 자막 텍스트 (AI 추천 시)
...
- **자동 저장 안 함** — 사용자가 "임시 저장" 또는 "검수 요청" 클릭 시 콘텐츠 metadata에 포함
- ContentEditorPage의 `buildCreateData()` / `buildUpdateData()`에서 metadata 필드에 병합
- 기존 `ContentCreateData.metadata` / `ContentUpdateData.metadata` 타입 활용 (이미 `Record<string, unknown>`)

### 8-3. 근거

- 효과음/이모지는 콘텐츠에 종속적 데이터 → 별도 테이블 불필요
- JSONB는 배열/중첩 구조 지원 → 타임라인 데이터에 적합
- 기존 metadata 필드 활용 → DB 마이그레이션 불필요
- 플랫폼별 변형본(variant)에도 `metadata` 필드 있음 → 플랫폼별 다른 효과음/이모지 가능
...
```
ContentEditorPage (Form + 로컬 상태 관리)
  │
  ├─ Form 상태 (Ant Design Form)
  │   ├─ title, body, hashtags, scheduled_at
  │   ├─ platforms, channel_ids
  │   └─ media_urls
  │
  ├─ 로컬 상태 (useState)
  │   ├─ soundEffects: SoundEffectEntry[]  → EffectsPanel
  │   ├─ emojiOverlays: EmojiOverlayEntry[] → EmojiPanel
  │   ├─ variantStates: Record<string, VariantState>
  │   ├─ customizeMode: boolean
  │   └─ 각 모달/패널 open 상태
  │
  ├─ Props 전달
  │   ├─ mediaUrls            → MediaMainArea (업로드/플레이어/갤러리)
  │   ├─ form (FormInstance)  → ContentInfoModal (제목/본문/해시태그)
  │   ├─ assetId              → SubtitlePanel, ShortformPanel,
  │   │                         EffectsPanel, EmojiPanel
  │   ├─ platforms/variants   → PlatformSettingsModal
  │   └─ contentText          → AiAssistantPanel
  │
  └─ 저장 시 병합
      buildCreateData() / buildUpdateData() {
        ...기존 필드,
        metadata: {
          ...기존 metadata,
...
- 콘텐츠 정보, 플랫폼 설정은 **모달** (헤더 버튼으로 진입)
- 자막/효과음/이모지/숏폼/AI 어시스턴트는 **인라인 패널** (도구 바로 전환)
- 효과음/이모지는 콘텐츠 metadata JSONB에 저장 (DB 변경 없음)
...
### 부록 E 업데이트 포인트
- "플랫폼별 맞춤" 토글이 헤더의 "플랫폼 설정" 모달로 이동
- 콘텐츠 정보 수정이 모달 기반으로 변경
- 새 레이아웃 반영
...
| # | 협의 내용 | 반영 섹션 | 상태 |
|---|---|---|---|
| 1 | 영상 중심으로 재배치 | 2-1 원칙 1, 2-2 레이아웃 | ✅ |
| 2 | 자막/숏폼 별도 페이지 → 인라인 | 2-4, 2-7 인라인 패널 | ✅ |
| 3 | 효과음/이모지 누락 해결 | 2-5, 2-6 신규 패널 | ✅ |
| 4 | 이미지·텍스트는 모달 처리 | 2-3 콘텐츠 정보 모달, 2-9 플랫폼 설정 모달 | ✅ |
| 5 | 별도 UI 없이 재설계 (모달 활용) | 도구 바 + 모달 조합으로 단일 페이지 | ✅ |
| 6 | AI 제목 제안 본문 의존 버그 | 3-1 getContentText 개선 | ✅ |
| 7 | AI 버튼 중복 | 3-2 중복 제거 | ✅ |
| 8 | AI 해시태그 결과 미표시 | 3-3 AiSuggestionPanel 연결 | ✅ |
| 9 | 미디어 업로드→플레이어 전환 흐름 | 2-2 전환 흐름 명시 | ✅ |
| 10 | 효과음/이모지 데이터 저장 위치 | 8 metadata JSONB 정의 | ✅ |
| 11 | 기존 라우트 호환 | 6 래퍼 패턴 | ✅ |
